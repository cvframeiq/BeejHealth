...
import { resolveConsultPhotoUrls } from './helpers.jsx';
export const SOILS = [
...
export const INDIA_DISTRICTS = {
  ...
  'Odisha': [
    'Angul','Balangir','Balasore','Bargarh','Boudh','Cuttack','Deogarh',
...
/* ── Taluka Data — Maharashtra full + other states placeholder ── */
export const MAHARASHTRA_TALUKAS = {
  ...
  'Mumbai Suburban':['Panvel','Uran','Pen','Alibag','Khalapur','Karjat','Matheran'],
};
...
function getStateTalukas(state, district) {
  ...
  // Other states — generic placeholder talukas
  return [`${district} - Taluka 1`, `${district} - Taluka 2`, `${district} - Taluka 3`];
}

/* ── Legacy aliases (backward compatibility) ── */
export const DISTRICTS = INDIA_DISTRICTS['Maharashtra'];
export const TALUKAS   = MAHARASHTRA_TALUKAS['Pune'];
...
export const OTHER_CROP = { id:'others', name:'Others', emoji:'➕', health:70, stage:'General / Mixed' };
...
function normalizeReportAnswer(answer) {
  ...
  return answer.label || answer.text || answer.value || answer.id || '';
}

export function getReportQuestionRows({ cropId, answers = {}, aiResult = null }) {
  const rows = [];
  const addRows = (questions, limit) => {
    (questions || []).slice(0, limit ?? questions.length).forEach((q) => {
      const answerText = normalizeReportAnswer(answers[q.id]);
      if (answerText) {
        rows.push({
          id: q.id,
          question: q.text || q.question || q.id,
          answer: answerText,
        });
      }
    });
  ...
  if (cropId === 'coconut') {
    if (answers.cq1?.label) {
      rows.push({ id: 'cq1', question: COCONUT_Q1_DATA.text, answer: normalizeReportAnswer(answers.cq1) });
      addRows(BRANCH_QS[answers.cq1.id] || [], 4);
      return rows;
    }

    if (aiResult?.disease && COCONUT_DISEASE_QS[aiResult.disease]) {
      addRows(COCONUT_DISEASE_QS[aiResult.disease], 5);
    }
    return rows;
  }

  if (answers.q1?.label) {
    rows.push({ id: 'q1', question: Q1_DATA.text, answer: normalizeReportAnswer(answers.q1) });
    addRows(BRANCH_QS[answers.q1.id] || [], 4);
  }

  return rows;
}

export function buildReportAiRows({ reportD, effectiveTop3 = [], effectiveTreatments = [], effectiveUrgency = 'medium' }) {
  const rows = [];
  if (effectiveTop3?.length) {
    rows.push({ label: 'Top prediction', value: `${effectiveTop3[0].disease} (${effectiveTop3[0].confidence}%)` });
  }
  rows.push({ label: 'Diagnosis', value: reportD.disease });
  rows.push({ label: 'Scientific name', value: reportD.sci });
  rows.push({ label: 'Primary cause', value: reportD.cause });
  rows.push({ label: 'Immediate action', value: reportD.phases?.[0] || '—' });
  rows.push({ label: 'This season', value: reportD.phases?.[2] || reportD.phases?.[1] || '—' });
  if (effectiveTreatments?.length) {
    rows.push({ label: 'AI treatment plan', value: effectiveTreatments.join(' • ') });
  }
  if (effectiveUrgency) {
    rows.push({ label: 'Urgency', value: effectiveUrgency });
  }
  return rows;
}
...
export function buildReportSnapshot({
  ...
  const photoUrls = Array.isArray(photoUrlsOverride) && photoUrlsOverride.length
    ? photoUrlsOverride.filter(Boolean)
    : resolveConsultPhotoUrls(dbConsult);
  const photoIds = (dbConsult?.photoIds?.length ? dbConsult.photoIds : dbConsult?.photoId ? [dbConsult.photoId] : []).filter(Boolean);
  const photoCount = dbConsult?.photoCount || photoUrls.length || photoIds.length;
  const photos = photoUrls.map((url, index) => ({
    photoId: photoIds[index] || photoIds[0] || null,
    url,
    index: index + 1,
  }));
  const rows = questionRows || getReportQuestionRows({ cropId: dbConsult?.cropId || crop?.id, answers, aiResult });
  const summary = `${reportD.disease} detected for ${crop?.name || 'Crop'}. ${photoCount} photo${photoCount === 1 ? '' : 's'} and ${rows.length} question${rows.length === 1 ? '' : 's'} saved in the report.`;
  ...
  return {
    ...
    downloadedAt: null,
    consultationId: dbConsult?._id || null,
    cropId: dbConsult?.cropId || crop?.id || null,
    cropName: dbConsult?.cropName || crop?.name || 'Crop',
    cropEmoji: dbConsult?.cropEmoji || crop?.emoji || '🌱',
    photoCount,
    photos,
    questionRows: rows,
    aiRows: aiRows || buildReportAiRows({ reportD, effectiveTop3, effectiveTreatments, effectiveUrgency }),
    questionAnswers: answers,
    summary,
    reportData: {
      disease: reportD.disease,
      sci: reportD.sci,
      hindi: reportD.hindi,
      confidence: reportD.conf,
      severity: reportD.sev,
      areaAffected: reportD.aff,
      cause: reportD.cause,
      humidity: reportD.humidity,
      risk: reportD.risk,
      phases: reportD.phases,
      meds: reportD.meds,
      top3: effectiveTop3,
    ...
    source: aiResult?.disease ? 'ai' : 'photo',
  };
}
...
export const COCONUT_DISEASE_QS = {
  'Gray Leaf Spot': [{
    id: 'cgls1',
