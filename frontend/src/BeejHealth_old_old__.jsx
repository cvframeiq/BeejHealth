...
════════════════════════════════════════════════════════════════ */
function AIReportPage({selCrop,nav,toast,qAnswers}) {
  ...
  useEffect(()=>{
    ...
    const consultId = localStorage.getItem('bh_view_consult') || localStorage.getItem('bh_latest_consult');
    if(!consultId) return;
...
function BeejHealthApp() {
  ...
  const renderPage=()=>{
    try { switch(page){
      ...
      case 'ai-report':        return <AIReportPage selCrop={selCrop||CROPS[0]} nav={nav} toast={toast} qAnswers={qAnswers}/>;
